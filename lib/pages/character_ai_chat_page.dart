...
String _systemPromptFor(GeneratedImageItem item) {
  ...
  buf.writeln(
    'You are roleplaying as an anime character chatting with the user in a mobile app.',
  );
  if (item.characterName.isNotEmpty) {
    buf.writeln('Your name: ${item.characterName}.');
  }
  if (item.personality.isNotEmpty) {
    buf.writeln('Personality: ${item.personality}');
  }
  if (item.styleDescription.isNotEmpty) {
    buf.writeln('Background and setting: ${item.styleDescription}');
  }
  if (item.gender.isNotEmpty) {
    buf.writeln('Gender: ${item.gender}.');
  }
  if (item.tags.isNotEmpty) {
    buf.writeln('Style tags: ${item.tags.join(', ')}.');
  }
  buf.writeln(
    'Stay in character. Be natural and conversational. Keep answers concise unless the user asks for more detail.',
  );
  buf.writeln(
    'You must always reply in English only, even if the user writes in another language.',
  );
  return buf.toString();
}

class CharacterAiChatPage extends StatefulWidget {
  const CharacterAiChatPage({super.key, required this.item});

  final GeneratedImageItem item;

  @override
  State<CharacterAiChatPage> createState() => _CharacterAiChatPageState();
}

class _CharacterAiChatPageState extends State<CharacterAiChatPage> {
  ...
  String get _botId => _sessionIdFor(widget.item);

  String get _displayName {
    final n = widget.item.characterName.trim();
    return n.isNotEmpty ? n : 'Character';
  }

  Future<void> _loadHistory() async {
    final history = await ChatHistoryService.loadHistory(_botId);
    if (!mounted) return;
    final lastTime = await ChatHistoryService.getLastMessageTime(_botId);
    setState(() {
  ...
  Future<void> _saveHistory() async {
    await ChatHistoryService.saveHistory(_botId, _messages);
    await ChatHistoryService.saveSessionMeta(
      _botId,
      name: _displayName,
      avatarUrl: widget.item.url,
      location: 'Character',
      lastMessageTime: DateTime.now(),
      characterItemJson: jsonEncode(widget.item.toJson()),
    );
  }
  ...
  void initState() {
    ...
    _loadHistory().then((_) {
      ...
      if (_messages.isEmpty) {
        final greeting =
            'Hi! I\'m $_displayName. What would you like to talk about?';
        setState(() {
          _messages.add({
            'role': 'assistant',
            'content': greeting,
  ...
  void _onPresetTap(String question) {
  ...
  bool get _shouldShowPresetQuestions {
    ...
    return !hasUser && !_isLoading;
  }

  Widget _buildUserAvatar(double size) {
    if (_userAvatar.startsWith('http')) {
      return ClipOval(
        child: Image.network(
          _userAvatar,
          fit: BoxFit.cover,
    ...
    if (_userAvatar.startsWith('assets/')) {
      return ClipOval(
        child: Image.asset(
          _userAvatar,
          fit: BoxFit.cover,
  ...
  Widget _buildCharacterAvatar(double size) {
    return ClipOval(
      child: SizedBox(
        width: size,
        height: size,
        child: CharacterImageDisplay(
          imageRef: widget.item.url,
          fit: BoxFit.cover,
          width: size,
          height: size,
        ),
        ...
        title: Row(
          children: [
            CircleAvatar(
              ...
              child: _buildCharacterAvatar(32),
            ),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                if (index < _messages.length + (_isLoading ? 1 : 0)) {
                  ...
                  final isLast = index == _messages.length - 1 && !_isLoading;
                ...
                return Padding(
                  ...
                  child: Wrap(
                    ...
                    children: _presetQuestions.map((q) {
                      return ActionChip(
                        label: Text(q),
                        onPressed: () => _onPresetTap(q),
                        backgroundColor:
                            _kThemeColor.withValues(alpha: 0.15),
